...
function capturarBase64() {
  ...
  // Antes: MAX = 640
  const MAX = 1200  // ← Cambiado: más resolución
  ...
  // Antes: 0.6 (60% calidad)
  return tmpCanvas.toDataURL('image/jpeg', 0.9).split(',')[1]  // ← Cambiado: 90% calidad
}

// ── OCR via Edge Function ────────────────────────────────────
async function ocrEdge(imageBase64) {
  const res = await fetchConTimeout(CONFIG.EDGE_URL, {
    method: 'POST',
    headers: {
      'Content-Type':  'application/json',
      'Authorization': `Bearer ${CONFIG.SUPABASE_ANON_KEY}`
...
async function hashTexto(texto) {
  const fecha = new Date().toISOString().slice(0, 10) // "2026-04-11"
  const data = new TextEncoder().encode(normalizar(texto) + fecha)
  ...
  try {
    ...
    if (!titulares.length) {
      ...
      titulares.forEach(t => {
        ...
        div.innerHTML = `
          <div class="titular-texto">${escapeHtml(t.texto)}</div>
...
function renderComentarios(comentarios) {
  ...
  panelComents.innerHTML = comentarios.map(c => `
    <div class="comentario-item" data-id="${escapeHtml(c.id)}">
      <div class="comentario-body">
        <div class="comentario-autor">
          ${escapeHtml(c.autor || 'anon')}
          <span class="comentario-fecha">${formatFecha(c.created_at)}</span>
        </div>
        <div class="comentario-texto">${escapeHtml(c.comentario)}</div>
      </div>
      ${c.autor === usuario ? `<button class="btn-borrar" data-id="${escapeHtml(c.id)}" title="Borrar">✕</button>` : ''}
    </div>
  `).join('')
